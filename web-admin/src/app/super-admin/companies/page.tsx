...
export default function CompaniesManagementPage() {
  const [companies, setCompanies] = useState<UserRole[]>([]);
  ...
  useEffect(() => {
    const fetchCompanies = async () => {
      try {
        const result = await apiFetch('/admin/users?role=company');
        ...
        if (errorMessage.includes('No authorization') || errorMessage.includes('Invalid')) {
          ...
          setError(errorMessage || 'Failed to load companies');
        }
      } finally {
        setLoading(false);
      }
  ...
  return (
    <div className="max-w-7xl mx-auto space-y-8">
      <div className="flex justify-between items-end mb-6">
        <div>
          <h1 className="text-3xl font-bold text-slate-900">Corporate Agencies</h1>
          <p className="text-slate-500 mt-2">Manage registered company accounts and their verification statuses.</p>
        </div>
      </div>
      ...
      <div className="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
        ...
        {loading ? (
          <div className="p-12 text-center text-slate-500">Loading companies...</div>
        ) : (
            ...
            <table className="w-full text-left border-collapse min-w-max">
              ...
              <tbody>
                {companies.map((company) => {
                  const verificationStatus = company.profile?.verification_status || 'unverified';
                  return (
                    <tr key={company.user_id} className="border-b border-slate-100 hover:bg-slate-50 transition-colors">
                      <td className="py-4 px-6 font-medium text-slate-500 text-sm">{company.user_id.substring(0, 8)}...</td>
                      <td className="py-4 px-6 font-bold text-slate-800">{company.profile?.company_name || 'N/A'}</td>
                      <td className="py-4 px-6 text-slate-600">{company.profile?.contact_no || 'N/A'}</td>
                      <td className="py-4 px-6 text-slate-600">{company.profile?.city || 'N/A'}</td>
